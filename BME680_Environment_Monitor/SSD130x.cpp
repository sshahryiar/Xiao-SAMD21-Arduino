...
{
  _i2cPort = &Wire;
  _deviceAddress = SSD1306_I2C_ADDRESS;
}


void OLED::begin(TwoWire &wirePort, uint8_t deviceAddress)
{
  _i2cPort = &wirePort;
  ...
  write_byte(SSD1306_Set_Multiplex_Ratoled_CMD, SSD1306_CMD);
  ...
  write_byte((SSD1306_Set_COM_Output_Scan_Directoledn_CMD | SSD1306_Scan_from_COM63_to_0), SSD1306_CMD);
  ...
  write_byte(SSD1306_Set_Pre_charge_Peroledd_CMD, SSD1306_CMD);
...
{
  _i2cPort->beginTransmission(SSD1306_I2C_ADDRESS); 
  _i2cPort->write(control_byte);  
...
void OLED::Bitmap(int16_t xb, int16_t yb, int16_t xe, int16_t ye, const uint8_t *bmp_img)
{
	uint16_t s = 0;
